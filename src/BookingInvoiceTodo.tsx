...
// ─── Frontend Matching ────────────────────────────────────────────────────────
// GAS makeMatchKeys_ สร้าง keys ในรูป:
//   conf:HMXXXXXX            (conf code — exact match)
//   n:namepart|YYYY-MM-DD   (name part + ci ±2 days)
//   cr:YYYY-MM-DD|ROOM      (ci ±2 days + room number)
// React ใช้ matchKeys ที่ GAS ส่งมาโดยตรง fallback เท่านั้นถ้าไม่มี
...
function allNameParts(raw: string): string[] {
  return String(raw || '').trim()
    .split(/[\s,\/\\]+/)
    .map((p: string) => p.toLowerCase().replace(/[^a-z0-9ก-๙]/g, ''))
...
// fallback — ใช้เมื่อ GAS ไม่ส่ง matchKeys มา (format ตรงกับ makeMatchKeys_)
function buildFallbackKeys(guest: string, checkin: string, room: string, confCode?: string): string[] {
  const keys: string[] = [];
  // conf: HM codes
  const hmCodes = String(confCode || '').match(/HM[A-Z0-9]{6,}/gi) || [];
  hmCodes.forEach((c: string) => keys.push('conf:' + c.toUpperCase()));
  // name × dates
  const parts = allNameParts(guest);
  const ci    = normDate(checkin);
  const dates = ciDates(ci);
  parts.forEach((p: string) => dates.forEach((dt: string) => keys.push('n:' + p + '|' + dt)));
  // room × dates
  const rn = extractRoomNum(room);
  if (rn) dates.forEach((dt: string) => keys.push('cr:' + dt + '|' + rn));
  return keys;
}

function buildFallbackBookingKeys(b: BookingRaw): string[] {
  return buildFallbackKeys(b.guest, b.checkin, b.room, b.resId);
}

function buildFallbackInvoiceKeys(inv: InvoiceRaw): string[] {
  const guests = String(inv.guest || '').split(',');
  const rooms  = String(inv.room  || '').split(',');
  const keys: string[] = [];
  guests.forEach((g: string, i: number) => {
    buildFallbackKeys(g.trim(), inv.checkin, rooms[i] || rooms[0] || '', inv.bookingId)
...
function enrichData(raw: { today?: string; booking?: BookingRaw[]; invoice?: InvoiceRaw[]; bookings?: BookingRaw[]; ledger?: InvoiceRaw[] }): DashboardData {
  ...
  // Deduplicate bookings by resId (same guest can appear twice from sheet if row duplicated)
  ...
  const booking: BookingItem[] = bookingsRaw
    ...
    .map(b => ({
      ...
      matchKeys: b.matchKeys?.length ? b.matchKeys : buildFallbackBookingKeys(b),
    }));
  ...
  invoicesRaw.forEach(inv => {
    ...
    const item: InvoiceItem = {
      ...
      matchKeys: inv.matchKeys?.length ? inv.matchKeys : buildFallbackInvoiceKeys(inv),
    };
    invoice.push(item);
  });
